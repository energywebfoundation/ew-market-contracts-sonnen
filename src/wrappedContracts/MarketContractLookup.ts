    ...
    export class MarketContractLookup extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(MarketContractLookupJSON.abi, address) : new web3.eth.Contract(MarketContractLookupJSON.abi, MarketContractLookupJSON.networks.length > 0 ? (MarketContractLookupJSON.networks[0]) : null))
        this.web3 = web3
    }
	...
	async init(_assetRegistry:string,_marketLogicRegistry:string,_marketDB:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.init(_assetRegistry,_marketLogicRegistry,_marketDB)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.init(_assetRegistry,_marketLogicRegistry,_marketDB)
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.init(_assetRegistry,_marketLogicRegistry,_marketDB)
	...
	async update(_marketRegistry:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.update(_marketRegistry)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.update(_marketRegistry)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.update(_marketRegistry)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
